  ...
  return (
    <div className="relative min-h-screen bg-[var(--drab-dark-brown)] text-[var(--lion)] font-sans overflow-hidden">
      ...
      <AnimatePresence>
        {showGrid && (
          <motion.div
            className="grid grid-cols-1 md:grid-cols-3 gap-4 p-10 min-h-screen font-sans bg-[var(--drab-dark-brown)] text-[var(--lion)]"
            initial="hidden"
            animate="visible"
            variants={{
              visible: {
          ...
          >
            {/* 👉 You can now place each bento block with `variants={fadeUp}` below */}

            {/* Intro Section */}
            <motion.div {...fadeUp} className="col-span-2 p-6 rounded-2xl bg-[var(--lion)] text-[var(--drab-dark-brown)] hover:scale-[1.02] transition-transform duration-200 ease-out">
     ...
     I love building tools that feel personal. I'm currently exploring creative NLP, vector embeddings, and visual storytelling using code.
          ...
          {/* Contact Block */}
          <motion.div {...fadeUp} className="col-span-2 p-6 rounded-2xl bg-[var(--drab-dark-brown-2)] text-[var(--lion)] hover:scale-[1.01] transition-transform duration-200 ease-out">
            ...
            {/* 👉 Add all other blocks here using `motion.div` and `variants={fadeUp}` */}
